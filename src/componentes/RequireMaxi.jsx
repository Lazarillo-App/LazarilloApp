...
export default function RequireMaxi({ children }) {
  ...
  const check = async () => {
    ...
    try {
      ...
      // ✅ Sub-negocios creados desde agrupación heredan Maxi de la org
      // → los tratamos como configurados aunque no tengan credenciales propias
      const isSubBusiness = s?.created_from === 'from_group';
      const isConfigured  = !!s?.configured || isSubBusiness;

      setStatus({ loading: false, configured: isConfigured });
