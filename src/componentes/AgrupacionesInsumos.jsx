...
import {
  obtenerAgrupacionesInsumos, crearAgrupacionInsumos, eliminarAgrupacionInsumos
...
export default function AgrupacionesInsumos() {
  ...
  const loadInsumos = async () => {
    const url = new URL(`${API}/insumos`);
    url.searchParams.set('page', 1);
    url.searchParams.set('limit', 1000); // traemos varios para seleccionar
        ...
        <tbody>
          {grupos.map(g => (
            <tr key={g.id} style={{ borderBottom:'1px solid #eee' }}>
              ...
              <td style={{ padding:8 }}>{Array.isArray(g.insumos) ? g.insumos.length : 0}</td>
              <td style={{ padding:8 }}>
