...
// //     // Sin Agrupación: siempre usar visibleIds
// //     if (g && esTodoGroup(g)) {
...
// //     // ✅ Para agrupaciones reales: construir SIEMPRE desde agrupaciones (fuente de verdad)
// //     // NO depender de visibleIds como fuente primaria — visibleIds puede llegar tarde
// //     // como prop y causar renders con datos inconsistentes.
// //     // visibleIds se usa como intersección adicional solo cuando viene del padre
// //     // (p.ej. para discontinuados o filtros especiales).
...
// //     // Construir set desde agrupaciones directamente
// //     const s = new Set();
// //     arr.forEach((a) => { const id = safeId(a); if (id != null) s.add(id); });
// //     appIds.forEach(id => { const n = Number(id); if (n > 0) s.add(n); });

// //     // Si no hay artículos en agrupaciones pero visibleIds tiene datos, usarlos
// //     // (puede pasar en el primer render antes de que agrupaciones esté poblado)
// //     if (s.size === 0 && visibleIds instanceof Set && visibleIds.size > 0) {
// //       return visibleIds;
// //     }
...
// //             // articuloIds: los ids del nodo visible, filtrados por activeIds
// //             // para que solo incluya artículos que pertenecen a la agrupación actual
// //             // (evita que rubros parcialmente movidos sigan apareciendo completos)
// //             const articuloIdsRaw = (sub?.categorias || [])
// //               .flatMap((c) => c?.articulos || [])
// //               .map((a) => safeId(a))
// //               .filter(Boolean);
// //             const articuloIds = (activeIds instanceof Set && activeIds.size > 0)
// //               ? articuloIdsRaw.filter(id => activeIds.has(id))
// //               : articuloIdsRaw;
...
function SidebarCategorias({
  ...
  const treeByCategoria = useMemo(() => {
    ...
    const out = [];
    for (const [catName, arts] of catMap.entries()) {
      const filtered = !activeIds ? arts : arts.filter((a) => {
        const id = safeId(a);
        return id != null && activeIds.has(id);
      });
      if (filtered.length > 0) {
