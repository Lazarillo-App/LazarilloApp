...
// //   // bizId real de una agrupación: usa el business_id que viene del servidor
// //   // Las agrupaciones normales del principal tienen business_id = rootBizId
// //   // cuando se inyectan al subnegocio. Fallback a agrupBizId si no hay dato.
...
// //       // ✅ Excluir la agrupación actual: sus artículos deben estar disponibles
// //       // para crear sub-agrupaciones a partir de ella
// //       .filter((g) => Number(g.id) !== currentGroupId)
// //       .forEach((g) =>
...
// //               fromGroupBizId: !isTodo && agrupacionSeleccionada?.business_id
// //                 ? Number(agrupacionSeleccionada.business_id)
// //                 : null,
...
// //           // ✅ Primero quitar del origen — así agrupacionSeleccionada se actualiza
// //           // con los artículos correctos ANTES de que create regenere agrupacionesRich
// //           const fromGrpId = preselect?.fromGroupId;
// //           if (!isTodo && fromGrpId && movingIds.length) {
// //             onMutateGroups?.({
// //               type: 'remove',
// //               groupId: Number(fromGrpId),
// //               ids: movingIds,
// //             });
// //           }
...
// //           // ✅ Después agregar la nueva agrupación al estado
// //           onMutateGroups?.({
// //             type: 'create',
// //             id: Number(newId),
// //             nombre: nombreCreado,
...
function SubrubroAccionesMenu({
  ...
  async function ejecutarMover({ bizId: toBizId, groupId: toId, groupNombre }) {
    ...
    try {
      if (isSameBiz) {
        ...
        await httpBiz(`/agrupaciones/${toId}/articulos`, { method: 'PUT', body: { ids } }, toBizId);
        ...
        if (fromId) {
          ...
          try {
            // Intentar move-items masivo primero
            await httpBiz(`/agrupaciones/${fromId}/move-items`, { method: 'POST', body: { toId, ids } }, fromBizIdLocal);
          } catch {
            // Fallback: borrar uno por uno
            for (const id of ids) {
              try { await httpBiz(`/agrupaciones/${fromId}/articulos/${id}`, { method: 'DELETE' }, fromBizIdLocal); } catch { }
            }
          }
          onMutateGroups?.({ type: 'remove', groupId: fromId, ids });
        }
  ...
  async function toggleDiscontinuarBloque() {
    ...
    try {
      if (!isInDiscontinuadosView) {
        // DISCONTINUAR
        await httpBiz(`/agrupaciones/${discontinuadosId}/articulos`, { method: 'PUT', body: { ids } }, discBizId);
  ...
  return (
    <>
      ...
      <AgrupacionCreateModal
        ...
        groupName={subDisplayName}
        allowedIds={
          // Usar los IDs del subrubro como allowedIds para que el modal
          // muestre y preseleccione exactamente esos artículos
          preselect?.articleIds?.length
