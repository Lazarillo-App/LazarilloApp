...
const BRAND = { tinta: '#15213E', paper: '#F2F4F7', celesteProfundo: '#2492C8' };
...
function useOrgs() {
  ...
  const refetch = async (patch = {}) => {
    ...
    setState(next);
    try {
      const res = await AdminAPI.listOrganizations({ q: next.q, page: next.page, pageSize: next.pageSize });
      setState(s => ({ ...s, ...res }));
    } catch {}
  };
  useEffect(() => { refetch({}); }, []);
...
export default function AdminOrganizations() {
  const { state, setState, refetch } = useOrgs();
    ...
    <div style={{ fontFamily: "'Archivo', system-ui, sans-serif" }}>
      <div style={{ background: '#fff', padding: '20px 28px', borderBottom: '0.5px solid #e2e8f0' }}>
        ...
        <p style={{ margin: '2px 0 0', fontSize: 12, color: '#94a3b8' }}>
          {state.total} organizaciones registradas
        </p>
      </div>
        ...
        <div style={{ background: '#fff', borderRadius: 10, border: '0.5px solid #e2e8f0', overflow: 'hidden' }}>
          ...
          {state.rows.map(o => (
            <div
              ...
              onClick={() => nav(`/admin/organizaciones/${o.id}`)}
            >
              <div>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: BRAND.tinta }}>{o.display_name || o.name}</p>
                <p style={{ margin: 0, fontSize: 11, color: '#94a3b8' }}>ID: {o.id}</p>
              </div>
              <span style={{ fontSize: 12, color: '#64748b' }}>{o.business_count}</span>
